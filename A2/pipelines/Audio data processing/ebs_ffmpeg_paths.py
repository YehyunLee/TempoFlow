"""
Cross-platform resolution for ffmpeg / ffprobe executables.

Priority:
1. Environment override: ``EBS_FFMPEG_PATH`` / ``EBS_FFPROBE_PATH`` (full path or command name)
2. ``shutil.which("ffmpeg")`` / ``shutil.which("ffprobe")`` (normal PATH)
3. On Windows, a few common install locations (e.g. winget, manual zip to C:\\ffmpeg)
4. Fall back to ``"ffmpeg"`` / ``"ffprobe"`` so subprocess errors stay explicit

Set env vars when binaries are not on PATH (typical on some Windows setups).
"""

from __future__ import annotations

import os
import shutil
import sys
from pathlib import Path


def _candidate_from_env(env_name: str) -> str | None:
    raw = (os.environ.get(env_name) or "").strip()
    if not raw:
        return None
    expanded = Path(raw).expanduser()
    if expanded.is_file():
        return str(expanded.resolve())
    found = shutil.which(raw)
    if found:
        return found
    return raw


def _common_windows_ffmpeg_dirs() -> list[Path]:
    out: list[Path] = []
    pf = os.environ.get("PROGRAMFILES")
    pfx86 = os.environ.get("PROGRAMFILES(X86)")
    if pf:
        out.append(Path(pf) / "ffmpeg" / "bin")
    if pfx86:
        out.append(Path(pfx86) / "ffmpeg" / "bin")
    # Manual installs / course VMs
    out.append(Path(r"C:\ffmpeg\bin"))
    return out


def _first_existing_exe(dirs: list[Path], name: str) -> str | None:
    for d in dirs:
        for fname in (name, f"{name}.exe"):
            p = d / fname
            if p.is_file():
                return str(p.resolve())
    return None


def resolve_ffmpeg_executable() -> str:
    """Return path or command name to invoke ffmpeg."""
    c = _candidate_from_env("EBS_FFMPEG_PATH")
    if c:
        return c
    w = shutil.which("ffmpeg")
    if w:
        return w
    if sys.platform == "win32":
        hit = _first_existing_exe(_common_windows_ffmpeg_dirs(), "ffmpeg")
        if hit:
            return hit
    return "ffmpeg"


def resolve_ffprobe_executable() -> str:
    """Return path or command name to invoke ffprobe."""
    c = _candidate_from_env("EBS_FFPROBE_PATH")
    if c:
        return c
    w = shutil.which("ffprobe")
    if w:
        return w
    if sys.platform == "win32":
        hit = _first_existing_exe(_common_windows_ffmpeg_dirs(), "ffprobe")
        if hit:
            return hit
    return "ffprobe"
